;(function($){
	var methods = {
		...
		pageData: function(_v, _current){
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, _current,p);
		...
		dataRender: function(_v, _data){
			var opts = _v.data("options");
			//根据入参拼表头
			var cols=opts.colname.split(",");
			var titles='<div class="teble_head">';
			for(var i=0;i<cols.length;i++){
				titles += '<span class="tablehead'+i+'">'+cols[i]+'</span>';
			}
			titles+="</div>";
			//拼接表体
			var cells = '';
				for(var i=0;i<_data.length;i++){
					//传递参数
					var rowData = JSON.stringify(_data[i]).replace(/\"/g,"'");
					cells += '<div class="cc_cells"><a href="javascript:void(0)" onclick="openNotice('+rowData+')" ><span class="id">&nbsp;</span>';
					cells += '<span class="title">'+_data[i].noticeTitle+'</span>';
					cells += '<span class="publishtime">'+_data[i].publishTime+'</span>';
					cells += '<span class="publisher">'+_data[i].publisher+'</span>';
					cells += '</a></div>';
					
				}
			//var _table=titles+cells;
			var _table = cells;
			if(opts.htmlBox===''){
				//||(typeof(opts.htmlBox)!=='Object')
				var abx = _v.prev();
		...
		ajaxData: function(_url, _current,_p){
			/**
			...
			return (function(){
				...
				$.ajax({
					url: _url,
					type: 'get',
					data: {"page":_current,"pageRows":_p},
	                dataType: 'json',
	                cache : false,  
        			async : false,
	...
	$.fn.zPager.defaults = {
		...
		url: '', //ajax路由
		htmlBox: '<div id="notices"></div>' //ajax数据写入容器
	}
